jms_app.controller('CategoriasController', ['$scope', '$window', '$http', '$timeout', '$interval', '$location', '$ngConfirm', 'FileSetup', 'Dtables', 'Utils', 'Popup', function ($scope, $window, $http, $timeout, $interval, $location, $ngConfirm, FileSetup, Dtables, Utils, Popup) {
  ...
  $scope.list = async () => {
    ...
    $scope.categoriasList = await $scope.http.post($http, $scope, {}, `${$scope.base_url}/categorias/list`, $scope.token)
    $.apply()
  }
  ...
  $scope.acao = async (action, item) => {
    ...
    switch (action) {
      ...
      case 'edit':
        ...
        break;
      case 'upsert':
        await $scope.http.post($http, $scope, $scope.frmCategoria, `${$scope.base_url}/categorias/setup`, $scope.token)
        await $scope.list()
      ...
      case 'delete':
        ...
        if (confirm) {
          await $scope.http.post($http, $scope, item, `${$scope.base_url}/categorias/remove`, $scope.token)
          await $scope.list()
        }
  ...
  $scope.verifyExist = async (indice_, elamentId) => {
    ...
    $(`#${elamentId}`).html('')
    const response = await $scope.http.post($http, $scope, filter_, `${$scope.base_url}/categorias/verifyExist`, $scope.token)
